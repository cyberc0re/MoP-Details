...
DF.EncounterJournal = {
	...
	EJ_SelectInstance = EncounterJournal_DisplayInstance or function() return nil end,
...
DF.FRAMELEVEL_OVERLAY = 100
DF.FRAMELEVEL_BACKGROUND = 50
...
--> menus
	...
	function DF:CreateInCombatTexture (frame)
		...
		in_combat_background:SetTexture (.6, 0, 0, .1)
...
--> tutorials
	...
	function DF:ShowTutorialAlertFrame (maintext, desctext, clickfunc)
		...
		if (not TutorialAlertFrame) then
			...
			TutorialAlertFrame = CreateFrame ("frame", "DetailsFrameworkAlertFrame", UIParent, "MicroButtonAlertTemplate") -- "MicroButtonAlertTemplate"
...
function DF:CreateAnimation (animation, type, order, duration, arg1, arg2, arg3, arg4, arg5, arg6, arg7)
	...
	if (type == "ALPHA") then
		local anim2 = animation:CreateAnimation (type)
		anim2:SetOrder (order or animation.NextAnimation)
		anim2:SetChange (-arg1)
		anim2:SetDuration (0)

		anim:SetOrder ((order or animation.NextAnimation) + 1)
		anim:SetChange (arg2)
...
function DF:CreateBorderWithSpread (parent, alpha1, alpha2, alpha3, size, spread)
	...
	local border1 = parent:CreateTexture (nil, "background")
	border1:SetTexture (0, 0, 0, alpha1 or default_border_color1)
	border1:SetPoint ("topleft", parent, "topleft", -1 + spread, 1 + (-spread), 0, 0)
	border1:SetPoint ("bottomleft", parent, "bottomleft", -1 + spread, -1 + spread, 0, 0)
...
function DF:ReskinSlider (slider, heightOffset)
	if (slider.slider) then
		...
		do
			local upButton = _G[slider:GetName().."ScrollBarScrollUpButton"]
			...
			normalTexture:SetPoint ("topleft", upButton, "topleft", offset, 0)
			normalTexture:SetPoint ("bottomright", upButton, "bottomright", offset, 0)
			...
			pushedTexture:SetPoint ("topleft", upButton, "topleft", offset, 0)
			pushedTexture:SetPoint ("bottomright", upButton, "bottomright", offset, 0)

			local disabledTexture = upButton:GetDisabledTexture()
			...
			disabledTexture:SetPoint ("topleft", upButton, "topleft", offset, 0)
			disabledTexture:SetPoint ("bottomright", upButton, "bottomright", offset, 0)

			upButton:SetSize (16, 16)
			upButton:SetBackdrop ({edgeFile = [[Interface\Buttons\WHITE8X8]], edgeSize = 1, bgFile = "Interface\\Tooltips\\UI-Tooltip-Background"})
			upButton:SetBackdropColor (0, 0, 0, 0.3)
			upButton:SetBackdropBorderColor (0, 0, 0, 1)
		...
		--down button
		do
			local downButton = _G[slider:GetName().."ScrollBarScrollDownButton"]
			...
			normalTexture:SetPoint ("topleft", downButton, "topleft", offset, -4)
			normalTexture:SetPoint ("bottomright", downButton, "bottomright", offset, -4)
			...
			pushedTexture:SetPoint ("topleft", downButton, "topleft", offset, -4)
			pushedTexture:SetPoint ("bottomright", downButton, "bottomright", offset, -4)

			local disabledTexture = downButton:GetDisabledTexture()
			disabledTexture:SetTexture ([[Interface\Buttons\Arrow-Down-Disabled]])
			disabledTexture:SetTexCoord (0, 1, 0, .8)
			disabledTexture:SetAlpha (.5)

			disabledTexture:SetPoint ("topleft", downButton, "topleft", offset, -4)
			disabledTexture:SetPoint ("bottomright", downButton, "bottomright", offset, -4)

			downButton:SetSize (16, 16)
			downButton:SetBackdrop ({edgeFile = [[Interface\Buttons\WHITE8X8]], edgeSize = 1, bgFile = "Interface\\Tooltips\\UI-Tooltip-Background"})
			downButton:SetBackdropColor (0, 0, 0, 0.3)
			downButton:SetBackdropBorderColor (0, 0, 0, 1)

			--<Anchor point="TOP" relativePoint="BOTTOM"/>
			--downButton:SetPoint ("top", slider.ScrollBar, "bottom", 0, 0)
		end

		--
		local scrollBar = _G[slider:GetName().."ScrollBar"]
		--if the parent has a editbox, this is a code editor
		if (slider:GetParent().editbox) then
			scrollBar:SetPoint ("TOPLEFT", slider, "TOPRIGHT", 12 + offset, -6)
			scrollBar:SetPoint ("BOTTOMLEFT", slider, "BOTTOMRIGHT", 12 + offset, 6 + (heightOffset and heightOffset*-1 or 0))
		else
			scrollBar:SetPoint ("TOPLEFT", slider, "TOPRIGHT", 6, -16)
			scrollBar:SetPoint ("BOTTOMLEFT", slider, "BOTTOMRIGHT", 6, 16 + (heightOffset and heightOffset*-1 or 0))
		end

		scrollBar:GetThumbTexture():SetTexture (.5, .5, .5, .3)
		scrollBar:GetThumbTexture():SetSize (12, 8)

		--

		scrollBar:SetBackdrop ({edgeFile = [[Interface\Buttons\WHITE8X8]], edgeSize = 1, bgFile = "Interface\\Tooltips\\UI-Tooltip-Background"})
...
function DF:GetClassSpecIDs (class)
	return specIDs [strupper(class or "")]
end
...
DF.BattlegroundSizes = {
	[462] = 15, --Arathi Basin
	[444] = 10, --Warsong Gulch
	[402] = 40,	--Alterac Valley
	[541] = 15, --Eye of the Storm
	[483] = 40, --Isle of Conquest

	[611] = 15, --Slavery Valley
	[861] = 10, --Silvershard Mines
	[916] = 10, --The Battle for Gilneas
	[917] = 10, --Temple of Kotmogu
}
