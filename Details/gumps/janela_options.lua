...
function _detalhes:InitializeOptionsWindow()
	local DetailsOptionsWindow = g:NewPanel(UIParent, nil, "DetailsOptionsWindow", nil, 897, 592)
	...
	function f.RefreshWindow()
		if not DetailsOptionsWindow.instance then
			...
			_detalhes:OpenOptionsWindow(DetailsOptionsWindow.instance)
		end
	end
end
...
local menus2 = {
	...
	window.is_window_settings = is_window_settings
		...
		function window:create_line_background (frameX, label, parent)
			local f = CreateFrame ("Button", nil, frameX)
...
function window:CreateFrame18()
	...
	local frame18 = window.options [18][1]
		...
		--> current dps
			...
			local currentDPSTitleDesc = g:NewLabel (frame18, _, "$parentCurrentDPSTitleDescText", "CurrentDPSTitleDescTextLabel", "Show a frame with DPS done only in the last 5 seconds. Useful for arena matches and dungeons.", "GameFontNormal", 10, "white")
...
function window:CreateFrame1()
	...
	--> avatar button
		...
		frame1.avatarPreview:SetDrawLayer("ARTWORK")
		frame1.avatarNickname:SetDrawLayer("OVERLAY")
		frame1.avatarPreview2:SetDrawLayer("BORDER")
...
function window:CreateFrame13()
	...
	--> exclamation warning about an exception on this character for use profile on all characters
		local exclamation_frame = CreateFrame ("Button", nil, frame13)
...
function window:CreateFrame9()
		...
		--> wallpaper
			...
			local subMenu = {
				...
				["DRESSUP"] = {
					...
					{value = [[Interface\DRESSUPFRAME\DressUpBackground-DeathKnight1]], label = "Death Knight", onclick = onSelectSecTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS["DEATHKNIGHT"]},
			...
			local backgroundTable = {
				...
				{value = "DEATHKNIGHT", label = "Death Knight", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS["DEATHKNIGHT"]},
				{value = "DRUID", label = "Druid", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["DRUID"]},
				{value = "HUNTER", label = "Hunter", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["HUNTER"]},
				{value = "MAGE", label = "Mage", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["MAGE"]},
				{value = "MONK", label = "Monk", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS["MONK"]},
				{value = "PALADIN", label = "Paladin", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["PALADIN"]},
				{value = "PRIEST", label = "Priest", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["PRIEST"]},
				{value = "ROGUE", label = "Rogue", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["ROGUE"]},
				{value = "SHAMAN", label = "Shaman", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["SHAMAN"]},
				{value = "WARLOCK", label = "Warlock", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["WARLOCK"]},
				{value = "WARRIOR", label = "Warrior", onclick = onSelectMainTexture, icon = _detalhes.class_icons_small, texcoord = CLASS_ICON_TCOORDS ["WARRIOR"]},
			}
	...
	-- Wallpaper Settings
		...
		local L1 = frame9:CreateTexture (nil, "ARTWORK")
		...
		local L2 = frame9:CreateTexture (nil, "ARTWORK")
...
function window:CreateFrame10()
	...
	--> performance profiles
		...
		local PerformanceProfileOptions = {
			{value = "Raid25", label = Loc ["STRING_OPTIONS_PERFORMANCE_RAID25"], onclick = OnSelectProfileType, icon = [[Interface\PvPRankBadges\PvPRank15]], iconcolor = {1, .8, 0, 1}, texcoord = {0, 1, 0, 1}},
			{value = "Raid40", label = Loc ["STRING_OPTIONS_PERFORMANCE_RAID40"], onclick = OnSelectProfileType, icon = [[Interface\PvPRankBadges\PvPRank15]], iconcolor = {1, .8, 0, 1}, texcoord = {0, 1, 0, 1}},
...
-------- plugins
	...
	for absName, pluginObject in pairs (allplugins_toolbar) do
		...
		local bframe = CreateFrame ("Button", "OptionsPluginToolbarBG", frame4)
	...
	for absName, pluginObject in pairs (allplugins_raid) do
		...
		local bframe = CreateFrame ("Button", "OptionsPluginRaidBG", frame4)
		bframe:SetSize (640, 20)
		bframe:SetPoint ("topleft", frame4, "topleft", 10, y)
		bframe:SetBackdrop ({bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", tile = true, tileSize = 16, insets = {left = 1, right = 1, top = 0, bottom = 1}})
		bframe:SetBackdropColor (.3, .3, .3, .3)
	...
	for absName, pluginObject in pairs (allplugins_solo) do
		...
		local bframe = CreateFrame ("Button", "OptionsPluginSoloBG", frame4)
		bframe:SetSize (640, 20)
		bframe:SetPoint ("topleft", frame4, "topleft", 10, y)
		bframe:SetBackdrop ({bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", tile = true, tileSize = 16, insets = {left = 1, right = 1, top = 0, bottom = 1}})
		bframe:SetBackdropColor (.3, .3, .3, .3)
