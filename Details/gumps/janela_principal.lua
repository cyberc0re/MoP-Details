...
-- GLOBALS: C_Timer, Details, DetailsFramework, DetailsKeyBindScrollDown, DetailsKeyBindScrollUp, DetailsWindowGroupPopUp1, DetailsWindowLockPopUp1, GameCooltip
-- GLOBALS: GameCooltip2, GameCooltipFrame1, GameTooltip, GetScreenHeight, GetScreenWidth, GetTime, InCombatLockdown, IsInInstance, LibStub, UIParent
-- GLOBALS: UnitAffectingCombat, atan2, cos, error, getmetatable, ipairs, loadstring, math, pairs, pcall, setfenv, shift_monitor, sin, sqrt, string
-- GLOBALS: strsplit, table, table_deepcopy, time, tinsert, tonumber, tremove, type, unpack, wipe
-- GLOBALS: DETAILS_SEGMENTTYPE_DUNGEON_TRASH, DETAILS_SEGMENTTYPE_RAID_BOSS, DETAILS_SEGMENTTYPE_RAID_TRASH, TALENTS, _G
...
local update_line = function(self, target_frame)
	...
	local numInterim = _math_floor(distance/40)
...
local tempo_movendo, precisa_ativar, instancia_alvo, tempo_fades, nao_anexados, flash_bounce, start_draw_lines, instance_ids_shown
...
local icon_frame_on_enter = function(self)
	...
	if(actor) then
		...
		elseif(actor.name) then --ensure it's an actor table
			...
			if(diff and(diff == 1 or diff == 2 or diff == 3 or diff == 4) and(attribute == 1 or attribute == 2)) then
				...
				if(db) then
					...
					if(bestRank) then
						...
						--" .. _math_floor(bestRank[2] or 0) .. " ilvl" .. " |
...
do
	...
	--search key: ~tooltip
	local tooltip_anchor = CreateFrame("Button", "DetailsTooltipAnchor", UIParent)
...
-- background window config -------------------------------------------------------------------------------------------------------------------------------------------------
		...
		backgrounddisplay:EnableMouse(true)
...
function gump:CriaNovaBarra(instancia, index)
	...
	local icon_frame = CreateFrame("Button", "DetailsBarra_IconFrame_" .. instancia.meu_id .. "_" .. index, new_row.statusbar)
...
function _detalhes:GetSegmentInfo(index)
	...
	if(combat) then
		...
		if(combat.is_boss and combat.is_boss.name) then
			...
			if(b) then
				background = b
				background_coords = party_wallpaper_tex -- TODO
...
local build_segment_list = function(self, elapsed)
	...
	if(parameters_table[2] > 0.15) then
		...
		----------- segments
		local menuIndex = 0
		_detalhes.segments_amount = _math_floor(_detalhes.segments_amount)
		...
		local fill = _math_abs(filled_segments - _detalhes.segments_amount)
		segments_used = 0
				...
				if(thisCombat) then
					...
					if(thisCombat.is_boss and thisCombat.is_boss.name) then
						...
						if(_detalhes.tooltip.submenu_wallpaper) then
							...
							if(background) then
								CoolTip:SetWallpaper(2, background, raid_wallpaper_tex, segments_wallpaper_color, true)
							else
		...
		CoolTip:AddIcon([[Interface\QUESTFRAME\UI-Quest-BulletPoint]], "main", "left", 16, 16, nil, nil, nil, nil, "orange")
			...
			for i, segment in _ipairs(combats_added) do
				...
				if(segmentType == DETAILS_SEGMENTTYPE_RAID_TRASH or segmentType == DETAILS_SEGMENTTYPE_DUNGEON_TRASH) then
...
function _detalhes:CreateFakeWindow()
	...
	for i = 1, 5 do
		...
		b:SetSize(12, 12)
		b:SetPoint("bottomright", tb, "bottomright", -((_math_abs(i-6)-1)*11) - 1, 2)
...
local function click_to_change_segment(instancia, buttontype)
	if(buttontype == "LeftButton") then
		...
		local select_ = _math_abs(goal - total_shown)
